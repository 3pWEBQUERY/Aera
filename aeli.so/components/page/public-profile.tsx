...
import { cardIndexBySlug, toPageCards } from "@/lib/cards";
...
export async function profileMetadata(handle: string): Promise<Metadata> {
...
export async function PublicProfileView({
  ...
  handle: string;
  /** Der Slug aus der Adresse — leer heißt: die erste Karte. */
  card?: string;
  ...
  const page: PageData = {
    ...
    theme: profile.theme,
    cards: toPageCards(profile.cards, parseTheme(profile.rawTheme)),
    activeCardIndex: cardIndexBySlug(profile.cards, card),
  ...
  return (
    <>
      ...
      <ProfilePage page={page} mode="live" strings={strings} liveUrl={profileUrl(profile.handle)} />
